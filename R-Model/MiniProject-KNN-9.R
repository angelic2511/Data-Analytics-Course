...
# Create the data point for Record #9
new <- risk[9,]
record.9 <- new[,-3]
cll <- c(risk[,3])

# classify the k-nearest neighbor for Record #9
knn.record9 <- knn(train = risk[,c(1,2,4,5)],
           test = record.9,
           cl = cll,
           k = 3
)
...
together <- rbind(record.9,risk[-3])
record9.dist<-rdist(together)
record9.dist